...
sgx_status_t get_aborted_nums(sgx_enclave_id_t eid, int* retval)
{
	sgx_status_t status;
	ms_get_aborted_nums_t ms;
	status = sgx_ecall(eid, 2, &ocall_table_Enclave, &ms);
	if (status == SGX_SUCCESS && retval) *retval = ms.ms_retval;
	return status;
}

sgx_status_t elall_put_one(sgx_enclave_id_t eid, int tid, int n)
{
	sgx_status_t status;
	ms_elall_put_one_t ms;
	ms.ms_tid = tid;
	ms.ms_n = n;
...
sgx_status_t ecall_vector(sgx_enclave_id_t eid)
{
	sgx_status_t status;
...
sgx_status_t ecall_queue(sgx_enclave_id_t eid)
{
	sgx_status_t status;
